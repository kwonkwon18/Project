...
$(document).ready(function() {
	$.ajax("/futsal/countOfAlarm", {
		...
		success: function(data) {
			...
			console.log("풋살" + data.confirmationTotal)
			$("#NumberOfAlarm").html("❕");
		}
	})
});
...
$("#alarmList").click(function() {
	$.ajax("/futsal/alarm", {
		...
		success: function(data) {
			...
			// 기존 내용 삭제
			$("#futsalHostAlarm").empty();
			$("#futsalMemberAlarm").empty();
			...
			alarmList.forEach(function(item) {
				...
				$("#futsalHostAlarm").append(`

    <div id = "futsalBoard${boardId}" class="btn btn-outline-dark mb-3" style="width: 650px;">
          ...
          `);
			...
			memberAlarmList.forEach(function(item) {
				...
				if (participation === 1 && userId != memberId) {
					...
					$("#futsalMemberAlarm").append(`
<div id = "futsalBoard${boardId}" class="btn btn-outline-primary mb-3" style="width: 650px; display: flex; ">
...
`);
					...
					$("#futsalMemberAlarm").append(`
<div id = "futsalBoard${boardId}" class="btn btn-outline-danger mb-3" style="width: 650px; display: flex; ">
    <div id="alarmDiv${boardId}" class="d-flex align-items-center" style="padding-right: 10px; padding-left: 10px;">
        🏃‍♀️ ${title} 신청이 ${message}
    </div>
</div>
...
$("#futsalHostAlarm").on("click", ".agreeParty", function() {
...
$("#futsalHostAlarm").on("click", ".disagreeParty", function() {
	var memberId = $(this).data('board-memberid');
	var userId = $(this).data('board-userid');
	var boardId = $(this).data('board-boardid');
	var title = $(this).data('board-title');
...
$("#futsalMemberAlarm").on("click", ".memberConfirmation", function() {
	var memberId = $(this).data('board-memberid');
	var userId = $(this).data('board-userid');
	var boardId = $(this).data('board-boardid');
	var title = $(this).data('board-title');
...
$("#futsalMemberAlarm").on("click", ".justConfirmation", function() {
	var memberId = $(this).data('board-memberid');
	var userId = $(this).data('board-userid');
	var boardId = $(this).data('board-boardid');
	var title = $(this).data('board-title');
...
$("#futsalMemberAlarm").on("click", ".poserConfirmation", function() {
	var memberId = $(this).data('board-memberid');
	var userId = $(this).data('board-userid');
	var boardId = $(this).data('board-boardid');
	var title = $(this).data('board-title');
